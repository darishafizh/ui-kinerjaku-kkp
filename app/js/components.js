...
const UI = {
      ...
      <div class="toolbar">
        <div class="toolbar-left">
          <div class="search-bar">
            ...
            <input type="text" placeholder="${searchPlaceholder || 'Cari...'}" oninput="UI.filterNearestTable(this)" />
          </div>
        </div>
  ...
  filterNearestTable(input) {
    const toolbar = input.closest('.toolbar');
    if (!toolbar) return;
    const container = toolbar.parentElement;
    if (!container) return;
    const tables = container.querySelectorAll('table tbody tr');
  ...
  paginateTable(select) {
    const container = select.closest('div[style]')?.parentElement?.parentElement;
    if (!container) return;
    const rows = container.querySelectorAll('table tbody tr');
    const limit = parseInt(select.value) || 25;
    let count = 0;
    rows.forEach(row => {
      count++;
    ...
    // Update "Showing" text
    const info = container.querySelector('div[style*="border-top"]');
    if (info) info.textContent = `Showing 1 to ${Math.min(limit, rows.length)} of ${rows.length} entries`;
  }
};
